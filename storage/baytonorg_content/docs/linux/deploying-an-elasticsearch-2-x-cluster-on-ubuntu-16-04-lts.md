...
## [1\. What is Elasticsearch?](#1-what-is-elasticsearch) {#1-what-is-elasticsearch}
...
### [1.1. Why a cluster?](#1-1-why-a-cluster) {#1-1-why-a-cluster}
...
## [2\. The environment](#2-the-environment) {#2-the-environment}

### [2.1. Hardware](#2-1-hardware) {#2-1-hardware}
...
### [2.2. Launching servers in a hosted environment](#2-2-launching-servers-in-a-hosted-environment) {#2-2-launching-servers-in-a-hosted-environment}
...
### [2.3. Deploying servers on LXD](#2-3-deploying-servers-on-lxd) {#2-3-deploying-servers-on-lxd}

If you have servers setup already with dedicated IPs and a private network, skip this step. If you haven't yet set up LXD, [follow my guide](/docs/linux/lxd-zfs-and-bridged-networking-on-ubuntu-16-04-lts/).
...
#### [2.3.1. LXD host VLAN setup](#2-3-1-lxd-host-vlan-setup) {#2-3-1-lxd-host-vlan-setup}
...
`lxc profile create **nics** && lxc profile edit **nics**`
...
## [3\. Preparation and installation](#3-preparation-and-installation) {#3-preparation-and-installation}

The following steps will need to be undertaken on all three containers **except where stated**.

### [3.1. Connect to the server](#3-1-connect-to-the-server) {#3-1-connect-to-the-server}
...
#### [Disable the root account](#disable-the-root-account) {#disable-the-root-account}
...
### [3.2. Server VLAN setup](#3-2-server-vlan-setup) {#3-2-server-vlan-setup}
...
### [3.3. Install Java](#3-3-install-java) {#3-3-install-java}
...
#### [3.3.1. OpenJDK](#3-3-1-open-jdk) {#3-3-1-open-jdk}
...
#### [3.3.2. Oracle Java](#3-3-2-oracle-java) {#3-3-2-oracle-java}
...
### [3.4. Install Elasticsearch](#3-4-install-elasticsearch) {#3-4-install-elasticsearch}
...
### [3.5. Configure Elasticsearch](#3-5-configure-elasticsearch) {#3-5-configure-elasticsearch}
...
## [4\. Using Elasticsearch](#4-using-elasticsearch) {#4-using-elasticsearch}
...
### [4.1. Create an index](#4-1-create-an-index) {#4-1-create-an-index}
...
### [4.2. Add data](#4-2-add-data) {#4-2-add-data}
...
### [4.3. Searching for data](#4-3-searching-for-data) {#4-3-searching-for-data}
...
## [5\. Adding more nodes](#5-adding-more-nodes) {#5-adding-more-nodes}

Should you need to add more nodes to the cluster in the future, it's relatively straight forward:

### [5.1. Manually](#5-1-manually) {#5-1-manually}
...
### [5.3. Hosted environments with clone support](#5-3-hosted-environments-with-clone-support) {#5-3-hosted-environments-with-clone-support}
...
### [5.4. Additional master nodes (redundancy)](#5-4-additional-master-nodes-redundancy) {#5-4-additional-master-nodes-redundancy}
